...
def order_triphones(file_name):
    ...
    # lines_phones = [print(i, line.split("|", 1)[1].strip()) for i, line in enumerate(lines)]
    ...
    for line in lines_orig:
        ...
        if len(partes) == 2:
            if partes[1] in dict_final:
                ...
                numero = 0.01
            number_to_get = 10.01 - int(numero)
...
def get_percents_big():
    ...
    with open('../hist_big_orig', 'r') as f:
        ...
        values = []
        for key, value in list(phones_percents_big_dict.items()):
...
#
    ...
    with open('tmp/hist_small_ord', 'r') as f:
        ...
        cont = 0
        for key, value in list(phones_rels_small_dict.items()):
            #  compute phone values
...
def file_to_trie(lista):
    ...
    for line in lista:
        ...
        triphone = str(triphone.strip())
        value = (float(value), )
        Dict[triphone] = value

    # return marisa_trie.BytesTrie(Dict.items())
    return marisa_trie.RecordTrie("<f", list(Dict.items()))
...
#    print "time: " + datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    #########################################end auto balancing
        ...
        while valid_selection == False:
            try:
                selected_sent_list = selected_sent_input.split(",")
                valid_selection=validate_input(num_sents, selected_sent_list)
        ...
        while not valid:
            ...
            if (choice in ['a', '1'] and len(selected_sent_list) == 1):
                ...
                for selected_sent in selected_sent_list:
...
def validate_input(max, sents):
    result = False
    try:
        for x in sents:
            y = x.strip()
            if (int(y) >= 0 and int(y) < max):
                result = True
            else:
                return False
        return result
